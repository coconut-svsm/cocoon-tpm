...
use crate::{
    crypto::{symcipher, CryptoError},
    fs::{
        cocoonfs::{alloc_bitmap, extent_ptr, CocoonFsFormatError},
        NvFsError,
    },
...
pub struct ImageLayout {
   ...
   /// [`Allocation Blocks`](Self::allocation_block_size_128b_log2).
    ...
    /// Base-2 logarithm of an allocation bitmap file block's size as specified
    ...
    ///
    /// For the time being,
    /// [`preauth_cca_protection_hmac_hash_alg`](Self::preauth_cca_protection_hmac_hash_alg) is
    /// forced to equal
    ...
    /// application use case, this constraint might get relaxed in the
    /// future. Even more so as domain specific encryption keys are being
    /// used for the CCA-protected entities, the latter of which, by themselves,
    /// might be of limited value if revealed.
